...
<?php 
$id = $_GET['id'];
    $req = $db->prepare('SELECT * FROM employes WHERE cin=?');
    $req->execute([$id]);
    $result = $req->fetch();
if(isset($_POST["valider"])){
    ...
    $req = $db->prepare('UPDATE employes set nom=? WHERE cin=?');
    $req->execute([$nom,$id]);
    $res = $db->prepare('UPDATE employes set cin=? WHERE cin=?');
    $res->execute([$cin,$id]);
    $ren = $db->prepare('UPDATE employes set prenom=? WHERE cin=?');
    $ren->execute([$prenom,$id]);
    $rem = $db->prepare('UPDATE employes set date_debut=? WHERE cin=?');
    $rem->execute([$date_debut,$id]);
    $ree = $db->prepare('UPDATE employes set nombre_absence=? WHERE cin=?');
    ...
    <div class="main_content" id="main-content">
        <div class="left_sidebar">
            <nav class="sidebar">
                <ul id="main-menu" class="metismenu">
                    ...
                    <li><a href="./index.php"><i class="fa fa-user"></i><span>Les employés</span></a></li>
                    <li><a href="../Stagiaires/index.php"><i class="fa fa-user"></i><span>Les stagiaires</span></a></li>
                </ul>
            </nav>
        </div>
        ...
        <div class="page">
            <nav class="navbar navbar-expand-lg navbar-light bg-light">
                <a class="navbar-brand" href="javascript:void(0);">Modifier les informations des employés</a>
            </nav>
            <div class="container-fluid">
                <div class="row clearfix">
                    <div class="col-lg-12 col-md-12">
                        <div class="card">
                            <div class="header">
                                <h2>modifier les informations de employe</h2>
                            </div>
                            <div class="body">
